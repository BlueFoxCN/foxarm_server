...
def grasp_sample(raw_data, grasp_num, save_dir='whole_pro_data'):
    ...
    for i in range(grasp_num):
        while True:
            ...
            if angle > 150 and distance <= GRIPPER_WIDTH_IN_PIXEL:
                candidate_grasps.append(sample)
                break

    return candidate_grasps, binary

def visualize_grasp(samples, binary, save_dir='whole_pro_data'):
    # visulize grasp
    ...
    for i, sample in enumerate(samples):
        ...
        for pt in sample:
            ...
            cv2.rectangle(color_vis,
                          ...
                          (255, 0, 0),
                          2)
        cv2.imwrite(save_dir + '/color_vis_%02d.png' % i, color_vis)

def depth2cloud(sample, raw_data):
    # grasp pixel position to camera coordinate 
...
def align(raw_data, samples, table_height=550, z_num=3, save_dir='whole_pro_data'):
    ...
    for i, sample in enumerate(samples):
        ...
        misc.imsave(save_dir + '/resize_image_%02d.png' % i, resize_image.data)
        # plt.imshow(resize_image.data, cmap='gray')
        # plt.show()
